...
function createBundleInput(bundleId: string, mode: number, deadline: bigint): string {
...
test("simulates createBundle -> dynamic candidate wallet discovery without an event topic", () => {
  for (const [bundleId, mode, deadline] of [["11".repeat(32), 1, 1_800_000_000n], ["22".repeat(32), 2, 1_900_000_000n]] as const) {
    const wallet = candidateWalletFromBundle(
      { from: CANDIDATE, to: MOTHERSHIP, input: createBundleInput(bundleId, mode, deadline) },
      MOTHERSHIP,
      { status: "0x1", logs: [] },
    );
...
test("rejects another contract, another method, and failed createBundle", () => {
  assert.equal(
    candidateWalletFromBundle({ from: CANDIDATE, to: "0x" + "11".repeat(20), input: "0x33b8ac0e" }, MOTHERSHIP, { status: "0x1", logs: [] }),
    null,
  );
